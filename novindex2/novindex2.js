...
// Рендер на подзаглавията (groups) – около продуктите + кошче и DnD
function renderSubheadingsForModerator(catKey) {
  const key = catKey || currentCat();                                  // работим с подадената или текущата категория
  const cat = CATALOG[key];
  if (!cat) return;
  ...
  let ref = titleEl;                                                    // ще вмъкваме след titleEl или след последното подзаглавие

  cat.groups.forEach((g, idx) => {
    const h = document.createElement("div");
    h.className = "sec-title";
    h.dataset.from = "mod";                                             // маркер за модераторските заглавия
    h.dataset.groupIndex = idx;                                         // индекс в cat.groups

    const textSpan = document.createElement("span");                    // вътрешен span за текста
    textSpan.className = "sec-title-text";
    textSpan.textContent = g.heading || `Подзаглавие ${idx + 1}`;

    const delBtn = document.createElement("button");                    // бутон за триене
    ...
    delBtn.addEventListener("click", (e) => {
      e.stopPropagation();                                              // да не trigger-ва drag
      const keyNow = currentCat();
      const catNow = CATALOG[keyNow];
      if (!catNow || !Array.isArray(catNow.groups)) return;

      const index = Number(h.dataset.groupIndex);
      if (!catNow.groups[index]) return;

      if (!confirm("Да изтрия ли това подзаглавие?")) return;

      catNow.groups.splice(index, 1);                                   // махаме group-а от данните
      persistDraft();                                                   // чернова → после „Запази всичко“ към Firestore
      renderSubheadingsForModerator(keyNow);                            // рисуваме наново
      enableSubheadingDnd();                                            // включваме пак DnD
    });
...
let draggedSub = null;                                                  // кое подзаглавие в момента влачим
...
function syncSubheadingOrder() {
  ...
  const old = cat.groups.slice();                                      // копие на стария масив
  const reordered = [];

  els.forEach((el, idx) => {
    const oldIdx = Number(el.dataset.groupIndex);
    if (old[oldIdx]) reordered.push(old[oldIdx]);                      // пълним в нов ред
    el.dataset.groupIndex = idx;                                       // обновяваме индекса в DOM
  });

  cat.groups = reordered;                                              // записваме новата подредба
  persistDraft();                                                      // чернова → после SAVE към Firestore
}
